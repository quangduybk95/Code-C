...
void insert(treetype *tree,elementtype res)
{
  ...
  if(*tree==NULL)
  ...
  else
    {
      ...
      strcpy(a,tmp->element.user);
      if(strcmp(res.user,a)<0) insert(&tmp->left,res);
      if(strcmp(res.user,a)==0) insert(&tmp->right,res);
      if(strcmp(res.user,a)>0) insert(&tmp->right,res);
...
treetype search(elementtype x,treetype tree)
{
  ...
  else if(strcmp(tree->element.user,x.user)==0)
    return tree;
  else if(strcmp(tree->element.user,x.user)<0)
...
void doimk(char mk[],elementtype x,treetype tree)
{
  if(strcmp(tree->element.user,x.user)==0)
    {strcpy(tree->element.pass,mk);
      printf("Doi mat khau thanh cong\n");}
  else if(strcmp(tree->element.user,x.user)<0)
    doimk(mk,x,tree->right);
  else
      doimk(mk,x,tree->left);
}
/*void inra(char mk[],treetype tree)
{
  if(strcmp(tree->element.user,mk)==0)
    {
      printf("Thong tin\n");
      printf("User : %s\n",tree->element.user);
      printf("Pass : %s\n",tree->element.pass);
  ...
  else if(strcmp(mk,tree->element.user)<0)
    doimk(mk,tree->right);
  else
      doimk(mk,tree->left);
      }*/
...
void print_LNR(treetype tree) //duyet theo thu tu giua. sap xep tang dan
{
  if(tree!=NULL)
    {
      print_LNR(tree->left);
      if(strcmp(tree->element.user,"Admin")!=0)
      printf("%-20s%-10s%-5.2f\n",tree->element.user,tree->element.pass,tree->element.diem);
...
void delete_node(elementtype x,treetype *tree)
{
  /*
  if(*tree!=NULL)
    if(strcmp(x.tu,(*tree)->element.tu)==-1)
      delete_node(x,&(*tree)->left);
    else if (strcmp(x.tu,(*tree)->element.tu)==1)
