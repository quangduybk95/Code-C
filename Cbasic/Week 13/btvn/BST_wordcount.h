...
void insert(treetype *tree,elementtype res)
{
  ...
  if(*tree==NULL)
    ...
    {
      node *tmp=*tree;
      strcpy(a,tmp->element.word);
      if(strcmp(res.word,a)<0) insert(&tmp->left,res);
      if(strcmp(res.word,a)==0) insert(&tmp->right,res);
      if(strcmp(res.word,a)>0) insert(&tmp->right,res);
    }
}
...
treetype search(elementtype x,treetype tree)
{
  ...
  else if(strcmp(tree->element.word,x.word)==0)
    return tree;
  else if(strcmp(tree->element.word,x.word)<0)
