...
node *create_node(keytype e)
{
  node *new;
  new=(node*)malloc(sizeof(node));
  if(new!=NULL)
    {
      new->left=NULL; new->right=NULL; new->key=e;
    }
  return new;
}
...
treetype add_left(treetype *tree, keytype res) //them 1 node vao trai nhat
{
...
treetype add_right(treetype *tree, keytype res) //them 1 node vao vi tri phai nhat.
{
  node *new=create_node(res);
  if(new==NULL) return (new);
  if(*tree==NULL)
...
//Nhap cay nhi phan.
treetype add_node(treetype *tree,keytype res)
{
  node *new=create_node(res);
  if(new==NULL) return (new);
  *tree=new;
}
void insert(treetype *tree,keytype res)
{
  int a;
  if(*tree==NULL)
    add_node(tree,res);
  else
    {
      node *tmp=*tree;
      a=tmp->key;
      if(res<a) insert(&tmp->left,res);
      if(res>=a) insert(&tmp->right,res);
    }
}
...
treetype create_from_2(keytype e, treetype l,treetype r)
{
  treetype N;
...
//Tim kiem tren BST
treetype search(keytype x,treetype tree)
{
...
//DELETE
void delete_node(keytype x,treetype *tree)
{
  if(*tree!=NULL)
    if(x<(*tree)->key)
